...
#define DHTPIN 4
#define DHTTYPE DHT11 // Assuming DHT11 as a common default. Change to DHT22 if using DHT22.
...
dht.begin();
Serial.println("DHT sensor initialized on pin 4 (type DHT11 assumed).");
...
if (millis() - lastDHTRead >= DHT_INTERVAL) {
    ...
    if (isnan(humidity) || isnan(temperature)) {
        ...
        // Optionally publish an error message to MQTT
        // mqttClient.publish("uiot/dht/status", "Failed to read sensor");
        return;
    }

    String payload = "{\"temperature\":" + String(temperature, 2) + ",\"humidity\":" + String(humidity, 2) + "}";
    String topic = "uiot/dht/data"; // Generic topic for DHT sensor data
    
    if (mqttClient.publish(topic.c_str(), payload.c_str())) {
        Serial.println("Published DHT data to " + topic + ": " + payload);
    } else {
        Serial.println("Failed to publish DHT data to " + topic);
    }
}
...
// Example of handling a command to trigger an immediate DHT read
if (strcmp(topic, "uiot/dht/commands") == 0) {
    if (strcmp(message, "READ") == 0) {
        Serial.println("Received 'READ' command for DHT sensor.");
        float humidity = dht.readHumidity();
        float temperature = dht.readTemperature();

        if (isnan(humidity) || isnan(temperature)) {
            Serial.println("Failed to read DHT sensor for command!");
            mqttClient.publish("uiot/dht/status", "Error reading sensor on demand.");
        } else {
            String payload = "{\"temperature\":" + String(temperature, 2) + ",\"humidity\":" + String(humidity, 2) + "}";
            String dataTopic = "uiot/dht/data";
            if (mqttClient.publish(dataTopic.c_str(), payload.c_str())) {
                Serial.println("Published commanded DHT data to " + dataTopic + ": " + payload);
            } else {
                Serial.println("Failed to publish commanded DHT data.");
            }
        }
    } else {
        Serial.println("Unknown command for DHT sensor: " + String(message));
    }
}
}
